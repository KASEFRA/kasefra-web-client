...
/**
 * Real Estate Account Creation Dialog
 * Single-step form for real estate accounts (no type-specific details)
 */
...
export function CreateRealEstateDialog({
  open,
  onOpenChange,
}: CreateRealEstateDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.success('Real estate account created successfully!')
      onOpenChange(false)
      form.reset()
      ...
      console.error('Error creating real estate account:', error)
      toast.error(
        error.response?.data?.detail || 'Failed to create real estate account'
      )
    } finally {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Create Real Estate Account</DialogTitle>
              <DialogDescription>
                Add a property or real estate asset
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
